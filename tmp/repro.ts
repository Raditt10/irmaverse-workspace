  ...
  try {
    ...
    if (recentActivityLogs.length === 0) {
        ...
        recentQuizzes.forEach(qa => {
            console.log("Quiz:", qa.material_quizzes?.title);
        });
    }

    const earnedBadges = await prisma.user_badges.findMany({
      where: { userId },
      include: { badges: true }, // Wait, in the route it uses 'badge', let me check schema again
...
// I need a valid userId. I'll search for one.
async function run() {
